...
class AdressbookORM:
    ...
    class ContactORM(db.Entity):
        ...
        def get_model(self):
            return Contact(id=self.id, firstname=self.firstname, middlename=self.middlename, lastname=self.lastname)#, footer=self.footer)
    ...
    def get_contacts_in_group(self, group):
        dbgroup = select(c for c in self.GroupORM if c.id == group.id).first()
