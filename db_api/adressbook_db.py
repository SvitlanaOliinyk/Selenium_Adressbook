...
class AdressbookDB:
    ...
    def get_group_list(self):
        with self.connection.cursor() as cursor:
            ...
            from group_list order by group_name, group_id"""
            cursor.execute(sql)
            group_list = []
            for row in cursor:
                group_list.append(Group(id=row[0], name=row[1], header=row[2], footer=[3]))
    ...
    def get_contact_list(self):
        with self.connection.cursor() as cursor:
            ...
            contact_list = []
            for row in cursor:
                contact_list.append(Contact(firstname=row[0], middlename=row[1], lastname=[2]))
        self.connection.commit()
        return contact_list
